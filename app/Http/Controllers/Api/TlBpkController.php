...
class TlBpkController extends Controller
{
    ...
    {
        // Ambil tahun & semester dari query param (optional)
        $tahun = $request->input('tahun');
        $semester = $request->input('semester');

        if (! $tahun || ! $semester) {
            // Default: Ambil data terbaru jika param tidak ada
        ...
        $baseQuery = TlBpk::where('tahun', $tahun)
        ...
        $list = collect($listPaginated->items())->map(function ($item) {
            return [
                'nama_skpd' => $item->nama_skpd,
                'persentase_tl' => $item->persentase_tl,
                'status' => $this->status($item->persentase_tl),
